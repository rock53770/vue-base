    ...
    function Scroller(elem, settings) {
        ...
        */
        that.show = function (prevAnim) {
            ...
            html += (s.display != 'inline' ? '<div class="dwbc' + (s.button3 ? ' dwbc-p' : '') + '"><span class="dwbw dwb-c"><span class="dwb">' + s.cancelText + '</span></span><span class="dwbw dwb-s"><span class="dwb">' + s.setText + '</span></span>' + (s.button3 ? '<span class="dwbw dwb-n"><span class="dwb">' + s.button3Text + '</span></span>' : '') + '</div></div>' : '<div class="dwcc"></div>') + '</div></div></div>';
            dw = $(html);
...
(function ($) {
    ...
    */
    ms.parseDate = function (format, value, settings) {
        var def = settings.defaultValue ? new Date(settings.defaultValue) : new Date()//modify by dongxiaochai@163.com 为了初始化的时候按照默认值的日期精确时分秒
