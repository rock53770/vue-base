 ...
 *
 * modify by dongxiaochai@163.com
 * 扩展了插件增加了onScrolling属性，表示滚动中执行的事件处理函数
 */
	...
	iScroll = function (el, options) {
		...
		that.options = {
			...
			onBeforeScrollStart: function (e) { 
				// modify by dxx 2016-11-24 加一层判断，否则通知列表选中默认事件被阻止了
				var target = e.explicitOriginalTarget || e.target,
					nodeType =target.nodeName.toLowerCase(),
					excludeArr = [
						"select",
						"option",
						"input",
				...
				for(var i in excludeArr){
					if(excludeArr[i] && nodeType == excludeArr[i]){
						isExclude = true;
					}
				}
				if(!isExclude) {
					do{
						if(target.className.indexOf("not-prevent") > -1){
							isExclude = true;
							break;
						}
			...
			onScrollMove: null,
			onScrolling:null,//扩展的，滚动中事件处理函数
		...
		//modify by dongxiaochai@163.com 2015-12-24 取消默认刷新
		// that.refresh();
